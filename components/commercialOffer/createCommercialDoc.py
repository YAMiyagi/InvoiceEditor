...
def createCommercialDoc(doc, data, signPath, propsIndex, loadJson, date, months, qty, summ,tablesData, isMultiTable):
    ...
    doc.add_text(x=175, y=620, text="Коммерческое предложение", font="Arial-Thick", font_size = 15)
    doc.add_text(x=100, y=580,
                    text_area_width = 400,
                    text=f'Уважаемый {data["clientName"].get()}, Благодарим за интерес к нашим услугам. Мы предлагаем рассмотреть наше предложение по реализации {data["offer"].get()}:', 
                    font_size=11)
